...
def preprocess_datasets(datapath, mat_filename, batch_size=256, train_perc=0.6, val_perc=0.2, data_norm=True, x_scaler=None,
                        device = device, scale_type=0, scale=None, intercept=None, dki_norm_type='MinMax', dki_name='dki',
                        wmti_name='wmti_paras', wmti_ranges=[[0, 1], [0, 4], [0, 3], [0, 3], [0, 1]], 
                        dki_ranges=[[0,3], [0, 3], [0, 3],[0, 10], [0, 10], [0, 10]], dki_length=6, seed=94361705):
    '''
    output: train, val, test datasets
    each: input and target tensor pairs
    exmaple: 
    train[0]: (tensor([ 0.7696,  2.1042,  0.1024,  0.2498,  0.0322, 25.6437]), tensor([  0.9111,   2.1900,   1.4116,   1.0584, 0.4471]))
    '''
    datafile = os.path.join(datapath, mat_filename)
    mats = sio.loadmat(datafile)
    dki = mats[dki_name].astype(np.float32)
    dki = dki[:,-dki_length:]
    wmti = mats[wmti_name].astype(np.float32)

    if data_norm:
        #input scaling
        if (x_scaler == None) and (dki_norm_type != None):
            if dki_norm_type==3:
                dki= min_max_scale(dki, multipler=100, ranges=dki_ranges)
            else:
...
def preprocess_input_dataset(datapath, mat_filename, batch_size=256, x_scaler=None, dki_norm_type=3, 
                                dki_ranges=[[0,3], [0, 3], [0, 3],[0, 10], [0, 10], [0, 10]], device = device):
    '''
    '''
    datafile = os.path.join(datapath, mat_filename)
    mats = sio.loadmat(datafile)
