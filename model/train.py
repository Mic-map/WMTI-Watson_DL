...
def evaluate(encoder, decoder, criterion, input_tensor, target_tensor, output_length=None):
    with torch.no_grad():
        ...
        decoder_hidden = encoder_hidden[-1]
        decoder_cell = encoder_cell[-1]
        decoder_attentions = torch.zeros(bz, target_length, input_length)
    ...
    return loss.item(), decoder_outputs, index_in_batch, outputs, decoder_attentions
...
def trainIters(encoder, decoder, train_data_loader, n_epochs, logger, save_dir='', st_epoch=-1, val_data_loader=None,
    ...
    if save_dir:
        ...
        if "checkpoints" not in save_dir:
        ...
        for i, datas in enumerate(train_data_loader):
            ...
            if (itr % print_every == 0) or itr ==n_iters:
                ...
                if val_data_loader is not None:
                    ...
                    val_loss, val_pred, val_index, _, _ = evaluate(encoder, decoder, criterion, val_input, val_target, output_length)
            ...
            if (itr % plot_every == 0) or itr ==n_iters:
                ...
                if val_data_loader is not None:
                    ...
                    for ival, (xval, yval) in enumerate(val_data_loader):
                        vloss, _, _ , _, _ = evaluate(encoder, decoder, criterion, xval.to(device), yval.to(device))
                        vloss_total += vloss    
