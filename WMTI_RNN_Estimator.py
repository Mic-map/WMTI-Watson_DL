...
class WMTI_RNN_Estimator:
    ...
    def read_dki_nii(self, dki_path, output_path, wmti_path=None, dki_names=['md', 'ad', 'rd', 'mk', 'ak', 'rk'], 
        ...
        dki_list=[]
        fa_map, self.header, self.file_ext = _get_image_from_nii(os.path.join(dki_path, fa_filename))
        self.image_size = fa_map.shape
        fa = fa_map.reshape(-1)

        for dn in self.dki_names:
            dki_, _,_ = _get_image_from_nii(os.path.join(dki_path,f'{dn}.nii'))
            dki_vector = dki_.reshape(-1)
        ...
        if wmti_path is not None:
            ...
            for wn in self.wmti_names:
                wmti_, _,_ = _get_image_from_nii(os.path.join(wmti_path,f'{wn}.nii'))
                wmti_vector = wmti_.reshape(-1)
        ...
        if mask is not None: 
            roimask, _,_ = _get_image_from_nii(mask)
            roimask = roimask.reshape(-1)
    ...
    def embed(self, embedded_filename='embedded_data', embedding_file=None, ratio=5, seed=SEED):
        if embedding_file is None:
            ...
            if not os.path.exists(embedding_file):
                remote_url = 'https://drive.google.com/uc?id=1TjWvLo6S_clOqodEuF3TtbaowLfGYVdY&export=download'
                gdown.download(remote_url, embedding_file,quiet=False)
    ...
    def wmti_maps(self, save_path=None, filtering=False):
        ...
        for i, nm in enumerate(self.wmti_names):
            ...
            nib.save(newData, os.path.join(save_path, f"{nm}{self.file_ext}"))  
...
def _get_image_from_nii(nii_file):
    nii_bsname = os.path.basename(nii_file)
    if not (os.path.exists(nii_file) or '.gz' in nii_bsname):
        nii_file = nii_file + '.gz'
    nii = nib.load(nii_file)

    file_ext = '.nii.gz' if '.gz'in os.path.basename(nii_file) else '.nii'
    return nii.get_fdata(), nii.header, file_ext
